<?php
	...
	class Repository {
		...
		public function search($mot){
			...
			$sql = "SELECT pr.id as id, pr.name as name, pr.description, pr.image, cat.name as namecat FROM products as pr, categories as cat WHERE pr.category_id = cat.id AND pr.name LIKE :mot OR pr.description LIKE :mot OR pr.image LIKE :mot OR cat.name LIKE :mot";
